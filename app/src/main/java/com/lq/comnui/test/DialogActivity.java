...
public class DialogActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View view) {
        int id = view.getId();
        if (id == R.id.btn_bar) { //设置状态栏是否为浅色模式
            startActivity(new Intent(this, BarActivity.class));
        } else if (id == R.id.btn_dialog_message) {// 消息对话框
        ...
        } else if (id == R.id.btn_dialog_input) {// 输入对话框
        ...
        } else if (id == R.id.btn_dialog_menu) {// 底部选择框
            ...
            new MenuDialog.Builder(this)
                    ...
                    // 设置点击按钮后不关闭对话框
                    //.setAutoDismiss(false)
                    .setList(selectBeanList).setListener(new MenuDialog.OnListener<SelectBean>() {
        ...
        } else if (id == R.id.btn_dialog_center_menu) {// 居中选择框
            ...
            new MenuDialog.Builder(this).setGravity(Gravity.CENTER)
                    ...
                    // 设置点击按钮后不关闭对话框
                    //.setAutoDismiss(false)
                    .setList(selectList).setListener(new MenuDialog.OnListener<SelectBean>() {
        ...
        } else if (id == R.id.btn_dialog_single_select) {// 单选对话框
            new SelectDialog.Builder(this).setTitle("请选择你的性别").setList("男", "女")
                    // 设置单选模式
                    .setSingleSelect()
                    // 设置默认选中
                    .setSelect(0).setListener(new SelectDialog.OnListener<String>() {
        ...
        } else if (id == R.id.btn_dialog_more_select) {// 多选对话框
            new SelectDialog.Builder(this).setTitle("请选择工作日").setList("星期一", "星期二", "星期三", "星期四", "星期五")
                    // 设置最大选择数
                    .setMaxSelect(3)
                    // 设置默认选中
                    .setSelect(2, 3, 4).setListener(new SelectDialog.OnListener<String>() {
        ...
        } else if (id == R.id.btn_dialog_pay) {// 支付密码输入对话框
            new PayPasswordDialog.Builder(this).setTitle("请输入支付密码").setSubTitle("用于购买一个女盆友").setMoney("￥ 100.00")
        ...
        } else if (id == R.id.btn_dialog_address) {// 选择地区对话框
            new AddressDialog.Builder(this).setTitle("请选择地区")
                    // 设置默认省份
                    //.setProvince("广东省")
                    // 设置默认城市（必须要先设置默认省份）
                    //.setCity("广州市")
        ...
        } else if (id == R.id.btn_dialog_date) {// 日期选择对话框
            new DateDialog.Builder(this).setTitle("请选择日期")
                    // 确定按钮文本
                    .setConfirm("确定")
                    // 设置 null 表示不显示取消按钮
                    .setCancel("取消")
        ...
        } else if (id == R.id.btn_dialog_time) {// 时间选择对话框
            new TimeDialog.Builder(this).setTitle("请选择时间")
                    // 确定按钮文本
                    .setConfirm("确定")
                    // 设置 null 表示不显示取消按钮
                    .setCancel("取消")
        ...
        } else if (id == R.id.btn_dialog_custom) {// 自定义对话框
            new BaseDialog.Builder(this).setContentView(R.layout.dialog_custom)
                    //.setText(id, "我是预设置的文本")
                    .setOnClickListener(R.id.btn_dialog_custom_ok, (dialog, view1) -> dialog.dismiss()).addOnShowListener(dialog -> ComnToast.showMsg("Dialog  显示了")).addOnCancelListener(dialog -> ComnToast.showMsg("Dialog 取消了")).addOnDismissListener(dialog -> ComnToast.showMsg("Dialog 销毁了")).setOnKeyListener((dialog, event) -> {
                ComnToast.showMsg("按键代码：" + event.getKeyCode());
                return false;
            }).show();
        } else if (id == R.id.btn_dialog_pop) {// 菜单弹窗
            new MenuPopup.Builder(this).setList("选择拍照", "选取相册").setListener((MenuPopup.OnListener<String>) (popupWindow, position, s) -> ComnToast.showMsg(s)).showAsDropDown(btn_dialog_pop);
        }
    }
}